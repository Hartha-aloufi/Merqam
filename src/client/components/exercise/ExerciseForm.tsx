...
import { calculateExerciseResults } from '@/client/lib/utils';
...
export default function ExerciseForm({ exercise }: { exercise: Exercise }) {
	const [userAnswers, setUserAnswers] = useState<UserAnswers>({
		multiple_choice_answers: Array(
			exercise.multiple_choice_questions.length
		).fill(''),
		true_false_answers: Array(exercise.true_false_questions.length).fill(
			false
		),
		...
		<form onSubmit={handleSubmit} className="max-w-3xl mx-auto space-y-8">
			...
			{exercise.multiple_choice_questions.length > 0 && (
				<div className="space-y-8">
					...
					{exercise.multiple_choice_questions.map(
						(question, qIndex) => (
							<motion.div
								key={qIndex}
								initial={{ opacity: 0, y: 20 }}
								...
								className={cn(
									...
									submitted && 'ring-2',
									submitted &&
										userAnswers.multiple_choice_answers[
											qIndex
					...
					{exercise.true_false_questions.map((question, qIndex) => (
						<motion.div
							...
							className={cn(
								...
								submitted && 'ring-2',
								submitted &&
									userAnswers.true_false_answers[qIndex] ===
										question.answer
									? 'ring-green-500/30'
									: submitted
									? 'ring-red-500/30'
							...
							<div className="p-6">
								...
								<div className="space-y-3">
									{[true, false].map((value) => {
										const isSelected =
											userAnswers.true_false_answers[
												qIndex
											] === value;
										const isCorrect =
						...
						>
							<div className="p-6">
								...
								<Textarea
									...
									onChange={(e) => {
										const newAnswers = [
											...userAnswers.detailed_answers,
										];
										newAnswers[qIndex] = e.target.value;
										setUserAnswers({
											...userAnswers,
											detailed_answers: newAnswers,
										});
									}}
