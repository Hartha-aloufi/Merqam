...
export class DownsubScraper extends BaseScraper {
  async extractVideoInfo(
    ...
    try {
      ...
      const pageInfo = await page.evaluate(() => {
        const titleElement = document.querySelector(".v-card__title.title a");
      ...
      if (!pageInfo.title) {
        throw new Error("Could not extract video title from Downsub page");
      }
  ...
  async scrape(url: string, outputBasePath: string): Promise<TranscriptResult> {
    if (!url.includes("downsub") && url.includes("youtube")) {
      url = new URL(`https://downsub.com/?url=${encodeURIComponent(url)}`).href;
    }
