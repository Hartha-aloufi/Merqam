...
export function useReadingProgressSync(topicId: string, lessonId: string) {
        ...
        mutationFn: (update: ReadingProgressUpdate) => {
            if (!isAuthenticated) {
                ...
                return new Promise((resolve) => {
                    setLessonProgress(topicId, lessonId, { paragraphIndex: update.latest_read_paragraph, date: "asdasd" });
                    resolve(undefined);
                });
            }
    ...
    const updateProgress = useCallback(debounce((latestIdx: number) => {
        progressMutation.mutate({ topic_id: topicId, lesson_id: lessonId, latest_read_paragraph: latestIdx });
    }, 7000), [])
