import { VideoTimeAt } from '@/client/components/video/VideoTimeAt';
import {
	Alert,
	AlertDescription,
	AlertTitle,
...
export const createMDXComponents = (indexGen: Generator<number>) => {
	return {
		...
		ul: ({
			...
			<ul
				className={cn('my-6 mr-6 space-y-3 list-disc', className)}
				{...props}
			/>
		),
		ol: ({
			...
			<ol
				className={cn('my-6 mr-6 space-y-3 list-decimal', className)}
				{...props}
			/>
		),
